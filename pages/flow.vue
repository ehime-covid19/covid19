      ...
      <div id="sy" :class="$style.section">
        ...
        <div :class="[$style.box, $style.bgGray]">
          ...
          <dl :class="$style.contact">
            ...
            <dd>
              <a :class="$style.tel" href="tel:089-909-3468">
                <icon-phone :class="$style.icon" aria-hidden="true" />
                089-909-3468</a
              >
            </dd>
...
export default Vue.extend({
  ...
  mounted() {
    ...
    this.buttons = Array.prototype.slice.call(
      document.getElementsByClassName((this as any).$style.anchorLink)
    )
    this.sections = Array.prototype.slice.call(
      document.querySelectorAll(`.${(this as any).$style.section}[id]`)
    )
    ...
    if (hash !== '') {
      ...
      document // eslint-disable-line no-unused-expressions
        .querySelector(`a.${(this as any).$style.anchorLink}[href='${hash}']`)
        ?.classList.add((this as any).$style.active)
  ...
  methods: {
      ...
      if (
        ...
        this.sections!.forEach(function(ele: HTMLElement, idx: number) {
          ...
          if (
            ...
            self.buttons![idx].classList.add((self as any).$style.active)
          } else if (
            self.buttons![idx].classList.contains((self as any).$style.active)
          ) {
            self.buttons![idx].classList.remove((self as any).$style.active)
          }
        })
      } else {
        this.stopFloating()
    ...
    onClickAnchor(event: Event): void {
      ...
      this.resetNavCurrent()
      target.classList.add((this as any).$style.active)
    ...
    startFloating(): void {
      if (!this.nav!.classList.contains((this as any).$style.floating)) {
        this.nav!.classList.add((this as any).$style.floating) // eslint-disable-line no-unused-expressions
      }
    ...
    stopFloating(): void {
      if (this.nav!.classList.contains((this as any).$style.floating)) {
        this.nav!.classList.remove((this as any).$style.floating) // eslint-disable-line no-unused-expressions
      }
    ...
    resetNavCurrent(): void {
      ...
      this.buttons!.forEach(function(ele: HTMLElement) {
        if (ele.classList.contains((self as any).$style.active)) {
          ele.classList.remove((self as any).$style.active)
        }
      })
    }
  },
